...
| verb-classification word lists (LOOKUP_WORDS/COMMIT_WORDS) and hedge list (HEDGE_RE) | HOMEGROWN heuristic lists, disclosed | affect ONLY the middle (verb) granularity and menu-prose disambiguation; coarse and exact granularities are list-free | no external standard exists; both lists are in behavior.py with tests |
